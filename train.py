...
from utils.glacier_loading import GlacierDataset, NormalizeTo0And255, ToTensor

mask_values = [0, 1, 2]
dir_img = Path("E:\\TESIS\\CONJUNTO_DE_DATOS\\{type}\\IMG")
dir_mask = Path("E:\\TESIS\\CONJUNTO_DE_DATOS\\{type}\\MASK")
...
def train_model(
    ...
    train_set = GlacierDataset(
        dir_img.replace("{type}", "TRAIN"),
        dir_mask.replace("{type}", "TRAIN"),
    ...
    logging.info(
        f"""Training and validation dataset sizes:
        Training: {n_train}
        Validation: {n_val}
    """
    )
    ...
    # (Initialize logging)
    experiment = wandb.init(project="glacier-unmsm", resume="allow", name="unet")
...
if __name__ == "__main__":
    ...
    model = UNet(n_channels=18, n_classes=args.classes, bilinear=args.bilinear)
